...
def extract_data(page_source):
    ...
    for config in configuration:
        ...
        if config.find('div', class_='label').text.strip() == 'Hộp số:':
            transmission = (
                        config.find('div', class_='txt_input') or config.find('div', class_='inputbox')).text.strip()
    ...
    raw_desc = description.find('div', class_='des_txt').get_text(separator=' ', strip=True)

    import re
    clean_desc = raw_desc.replace('\r', '').replace('\n', ' ').replace('\xa0', ' ')


    description = re.sub(' +', ' ', clean_desc).strip()

    return [brand, model, year, status, odo, origin, style, transmission, engine, exterior_color, interior_color, seats,
            doors, drive, description]
...
def crawl_page(page_source, index):
    ...
    dataframe_.columns = ['brand', 'model', 'year', 'status', 'odo', 'origin', 'style', 'transmission', 'engine',
                          'exterior_color', 'interior_color', 'seats', 'doors', 'drive', 'description']
